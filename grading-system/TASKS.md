...
### OCR 精度改善
- **フルモデル化**: `DocumentAnalyzer(configs={})` で parseq-large-v4.1 を使用（lite より大幅に精度向上）。
- **スペース区切り多文字ワード修正**: `'イ ウ エ エ'` のようにスペース含む文字列を `_assign_words_to_cells` で正しく分割。`chars = [c for c in text if not c.isspace()]` で空白除去後に比例マッピング。
- **前処理**: `Contrast×1.2 / Sharpness×1.5`（CLAHE は罫線ノイズ増幅リスクがあるため不採用）。
- **DPI=200**: DPI=300 は座標ズレが生じたため 200 に固定。

### OCR 補正マップ拡張（`backend/config.json`）
- `u→ウ / e→エ / i→イ`（小文字の明示マッピング）を追加。
- 手書きカタカナの代表的な誤読パターンを正解マッピングへ変更:
  - `P/p → ア`、`T → イ`、`t → ウ`、`H/h → エ`
  - `∧→ア`、`|→イ`、`∪→ウ`、`工/王→エ`
- 残りの全小文字アルファベット・数字（0,2〜6,8,9）を null マップ（要確認扱い）。
- `grading.py` にコードレベルの ASCII フォールバック追加: 日本語文字（ひらがな・カタカナ・漢字）を含まないテキストは自動で `review` に落とす。
- `judge_answers` の表示ロジック修正: ASCII 生テキスト（英数字の誤読）はセルに表示せず、空のオレンジセルで要確認を示す。

### Tab① 正解登録 UI 改善
- **オーバーレイ編集**: 正解用紙の PDF 画像上に各解答セルを SVG で重ねて表示。セルクリックでポップアップが開き、選択肢を変更すると即 `PATCH /api/answer-key/cell` で保存。
- **テキスト左寄せ**: OCR 判定テキストを枠内左側に表示（`text-anchor: start`）。
- **PDF ファースト UI**: ドロップダウングリッドと「修正を保存」ボタンを廃止し、サマリーバー（問題数・要確認数）＋凡例＋PDF オーバーレイのみのシンプル構成に変更。

### Tab② 採点・修正 UI
- **ページビュー**: 採点後に生徒答案 PDF の各ページ画像を表示（`data/sessions/{id}_imgs/page_{idx}.jpg`）。← → ボタンで前後ページを移動。
- **生徒情報バー**: 出席番号（クリックで編集）・氏名・得点をページ上部に表示。
- **SVG オーバーレイ編集**: 各解答セル上に judge 色（緑/赤/灰/橙）で結果を重ねて表示。クリックでポップアップを開き修正 → `PATCH /api/session/{id}/cell`。修正時に Tab③ グリッドも即時同期。
- **新エンドポイント追加**: `GET /api/session/{id}/page/{page_index}` でページ画像を返す。

### Tab③ 一覧・確認
- タブ名を「一覧・修正」→「一覧・確認」に変更。

### Tab④ 出力
- **採点済み PDF 出力** (`GET /api/session/{id}/scored-pdf`):
  - 各生徒の答案画像に `PIL.ImageDraw` で ○（正解・赤）/ ×（不正解・赤）を採点枠の左側に描画。
  - 正解以外（wrong / blank / review）はすべて × を付与。
  - 右上に合計点を赤枠付きで表示（例: `42 / 50`）。
  - 全ページを1つの PDF にまとめてダウンロード可能。
  - マークサイズ: セル高さの約 38%（小さめ）。

### プレビュー表示
- `.page-preview-wrap` の `max-height: 75vh; overflow: auto` を除去し、PDF ページ全体をスクロールなしで表示。
